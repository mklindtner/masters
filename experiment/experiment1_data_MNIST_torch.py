...
def setup_distillation_experiment(batch_size):
    ...
    # transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize( (0.0,), (126.0,))])
    # transform = transforms.Compose([
    #     transforms.ToTensor(), 
    #     transforms.Normalize( (0.0,), (1/2,)) #match dark knowledge paper
    ...
    trainset = torchvision.datasets.MNIST(root='./data', train=True, download=True, transform=transform_pipeline)
    
    train_loader = DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=0)
    # print(next(train_loader))
    # return
    testset = torchvision.datasets.MNIST(root='./data', train=False, download=True, transform=transform_pipeline)
